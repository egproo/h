    ...
    <div class="container mx-auto p-4 space-y-6">
        ...
        <div class="p-4 bg-white dark:bg-gray-800 shadow-lg rounded-lg flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0 md:space-x-6 transition-colors duration-300">
            ...
            <div class="flex-shrink-0 space-y-2 text-center">
			    <h2 class="text-sm text-gray-600 dark:text-gray-400">موفر الخدمة<h2>
                <div class="w-12 h-12 relative mx-auto">
                    <img style="width: 120px;text-align: center;margin: 0 auto;" src="{{ $appointment->provider->image }}" alt="{{ $appointment->provider->name }}" class="w-full h-20 rounded-full object-cover shadow-md">
                </div>
                <h3 class="mt-2 text-lg font-semibold text-gray-800 dark:text-gray-200">{{ $appointment->provider->name }}</h3>
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ $appointment->provider->title }}</p>
            </div>
            ...
            <div class="flex-grow space-y-2 pl-6 text-center md:border-l-2 border-gray-200 dark:border-gray-600">
                ...
                <div class="w-12 h-12 relative mx-auto">
                    <img style="width: 120px;text-align: center;margin: 0 auto;" src="{{url('/')}}/storage/{{ $appointment->service->image }}" alt="{{ $appointment->provider->name }}" class="w-full h-20 rounded-full object-cover shadow-md">
                </div>
        ...
        <div class="p-4 bg-white dark:bg-gray-800 shadow-lg rounded-lg transition-colors duration-300">
            @livewire('appointment-chat', ['appointmentId' => $appointment->id])
        </div>
    </div>
</x-filament-panels::page>
