...
{
    $providerId = Filament::auth('panel')->user()->id; // تغيير الواجهة إلى 'panel'

    return static::getModel()::query()
        ->leftJoin('appointments', 'appointments.id', '=', 'payment_attempts.appointment_id')
        ->where('appointments.provider_id', $providerId) // استخدام provider_id بدلاً من user_id
...
public static function getEloquentQuery(): Builder
{
    ...
    return parent::getEloquentQuery()
        ->whereHas('appointment', function ($query) use ($userId) {
            $query->where('provider_id', $userId)->where('is_successful', 1);
        })
